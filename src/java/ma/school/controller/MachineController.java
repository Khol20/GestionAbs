...
public class MachineController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (op.equalsIgnoreCase("envoyer")) {
            ...
            int id = Integer.parseInt(request.getParameter("id"));
            Machine machineToUpdate = mms.findById(id);
            ...
            if (machineToUpdate != null) {
                ...
                machineToUpdate.setReference(ref);
                machineToUpdate.setPrix(prix);
                machineToUpdate.setDateAchat(dateAchat);
                machineToUpdate.setMarque(marque);
                // Appeler la méthode de mise à jour
                mms.update(machineToUpdate);
                response.sendRedirect("machineForm.jsp");
            } else {
                response.sendRedirect("errorPage.jsp?message=La machine avec l'identifiant spécifié n'existe pas");
            }
        }
    }
