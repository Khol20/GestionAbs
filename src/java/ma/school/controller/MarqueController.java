...
public class MarqueController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (op.equals("Envoyer")) {
            ...
            int id = Integer.parseInt(request.getParameter("id"));
            Marque marqueToUpdate = ms.findById(id);
            
            if (marqueToUpdate != null) {
                String code = request.getParameter("code");
                String libelle = request.getParameter("libelle");
                // Mettre à jour les attributs de la marque
                marqueToUpdate.setCode(code);
                marqueToUpdate.setLibelle(libelle);
         

                // Appeler la méthode de mise à jour
                ms.update(marqueToUpdate);
                response.sendRedirect("marqueForm.jsp");
            } else {
                response.sendRedirect("errorPage.jsp?message=La marque avec l'identifiant spécifié n'existe pas");
            }
        }
